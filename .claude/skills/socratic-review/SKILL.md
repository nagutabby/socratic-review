...
- ユーザーの回答に応じて、専門家が指摘した対象ファイル・行番号に対し次のいずれかを行う：
  - **C（意図通り）を選択した場合:** 実装は変更せず、以下のスクリプトで該当箇所にその設計判断の理由を説明するコメントを追記する。
    - `.claude/skills/socratic-review/scripts/append-comment.sh "<ファイルパス>" "<行番号>" "<専門家名>" "<質問内容>" "<回答内容>" "intended"`
...
- Cを選択した問答は `append-comment.sh` により該当ファイルへ説明コメントとして記録済みのため追加の状態保存は不要。A/Bを選択した問答は、修正済みの実装差分そのものが記録となるため追加の状態保存・後処理は不要。
